...
// this exercises the real v2 schema end to end: graphql-js resolving the
// `AIAgentResponseSection` union against the payloads `runTurn` yields, and
// the real ArtworkType/ArtistType resolving their fields off raw Gravity
// hashes. It is the only place the wire shape a client selects is checked.
...
describe("aiAgentTurn sections", () => {
  ...
  it("resolves an artworks section, and the same works in legacy `artworks`", async () => {
    ...
    // The legacy field a pre-`sections` client selects still works, and shows
    // the same works.
  ...
  it("rejects an unknown section type at the schema boundary", () => {
    // The enum is what keeps a capability this server has never heard of off
    // the input surface in the first place.
  ...
  it("rejects an unknown entity type at the schema boundary", () => {
    // The enum is the input guard -- an entity type the server has no loader
    // for never reaches the resolver as a string to be defended against.
    const errors = validate(
      v2Schema(),
