...
// A section type a newer client knows about and this server doesn't. The
// GraphQL enum keeps it off the real input surface, hence the cast.
const UNKNOWN_SECTION = ("ARTISTS" as unknown) as AIAgentSectionType

// Most cases here are about loading, not capability.
const hydrate = (
...
describe("resolveSupportedSections", () => {
  ...
  it("offers nothing at all rather than falling back to the default", () => {
    // Substituting the default would answer a question the client didn't ask
    // and hide that it is out of sync.
...
describe("hydrateSection", () => {
  ...
  it("refuses to load a section the client can't render", async () => {
    // Unreachable in practice, since the output schema is built from the same
    // list -- but it must not spend Gravity calls on cards nobody will see.
    const artworksLoader = jest.fn()
