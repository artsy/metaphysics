...
export async function* runTurn(
  ...
  try {
    ...
    const result = streamText({
      ...
      // follow-up call from the same client. The capability block makes that
      // one entry per set rather than one shared entry: `system` takes a
      // single string, so there is no second breakpoint to split it on.
      system: {
        role: "system",
        content: system,
        providerOptions: {
