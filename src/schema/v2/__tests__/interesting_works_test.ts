...
describe("interestingWorks", () => {
  it("returns interesting works", async () => {
    const query = gql`
      {
        interestingWorks(first: 100) {
          totalCount
          edges {
            node {
              internalID
    ...
    const artworksLoader = jest.fn(async () => [
      {
        artist: {
    ...
    const { interestingWorks } = await runAuthenticatedQuery(query, context)

    expect(interestingWorks).toMatchInlineSnapshot(`
      Object {
        "edges": Array [
          Object {
            "node": Object {
