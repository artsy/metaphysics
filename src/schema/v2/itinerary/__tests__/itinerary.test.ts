...
// One Gravity payload, in the snake_case the endpoint actually returns. It
// carries every shape worth exercising: a stop backed by a show, one backed
// by a gallery location, and a custom stop with no item at all.
...
const loaders = () => ({
  ...
  partnerLocationByIdLoader: jest
    .fn()
    .mockResolvedValue({ _id: "location-1", city: "London" }),
...
describe("Itinerary", () => {
  ...
  // Without deriving the versions from those keys, `setVersion` finds none
  // and hands back the raw `:version` template.
  ...
  it("resolves each stop's item through the batched loaders", async () => {
    ...
    expect(context.partnerLocationByIdLoader).toHaveBeenCalledWith("location-1")
  })

  // Gravity 404s a private itinerary rather than 403ing, so it never
  // confirms one exists. The field resolves null rather than erroring.
