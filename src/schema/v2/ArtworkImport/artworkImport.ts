...
const ArtworkImportStatisticsType = new GraphQLObjectType({
  ...
  fields: {
    ...
    totalBlockingErrorsCount: {
      type: new GraphQLNonNull(GraphQLInt),
      description: "Total number of blocking errors across all rows",
      resolve: ({ total_blocking_errors_count }) => total_blocking_errors_count,
    },
    totalNonBlockingErrorsCount: {
      type: new GraphQLNonNull(GraphQLInt),
      description: "Total number of non-blocking errors across all rows",
      resolve: ({ total_non_blocking_errors_count }) =>
        total_non_blocking_errors_count,
    },
    uniqueRowsWithErrorsCount: {
      type: new GraphQLNonNull(GraphQLInt),
      description:
        'Total number of unique rows with errors (for "X artworks require attention")',
      resolve: ({ unique_rows_with_errors_count }) =>
        unique_rows_with_errors_count,
    },
    errorTypeCounts: {
      type: new GraphQLNonNull(
