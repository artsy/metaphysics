...
describe("Bidder position mutation", () => {
  ...
  describe("success", () => {
    it("creates a bidder position", async () => {
      const rootValue = {
        createBidderPositionLoader: sinon.stub().returns(Promise.resolve(createBidderPosition)),
      }
...
:{\"base\":[\"Please enter a bid higher than $37,000\"]}}"
      ...
      const rootValue = {
        createBidderPositionLoader: sinon.stub().returns(Promise.reject(errorMessage)),
      }
      const data = await runAuthenticatedQuery(query, rootValue)
 ...
 Bid again to take the lead.")
    ...
    it("creates correct message when sale is closed", async () => {
      ...
      expect(data.createBidderPosition.result.message_description_md).toEqual("Sorry, your bid wasn’t received before the lot closed.")
    })
  })
  it("creates correct message when live bidding has started", async () => {
    ...
    expect(data.createBidderPosition.result.message_description_md).toEqual(`Sorry, your bid wasn’t received before live bidding started.\
...
it("creates correct message when bidder is not qualifdied", async () => {
  ...
  expect(data.createBidderPosition.result.message_description_md).toEqual("Your bid can’t be placed at this time.\
