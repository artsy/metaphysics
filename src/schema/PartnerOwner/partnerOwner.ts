...
import { AlertType, PartnerAlertsEdgeFields } from "schema/v2/Alerts"
import { date } from "schema/v2/fields/date"
import { fields as partnerArtistFields } from "schema/v2/partner/partner_artist"
import { PartnerCategoryType } from "schema/v2/partner/partner_category"
import { PartnerDocumentsConnection } from "schema/v2/partner/partnerDocumentsConnection"
import { InquiryRequestType } from "schema/v2/partner/partnerInquiryRequest"
import {
  ViewingRoomsConnection,
  ViewingRoomStatusEnum,
} from "schema/v2/viewingRoomConnection"
...
export const PartnerOwnerType = new GraphQLObjectType<any, ResolverContext>({
  name: "PartnerOwner",
...
const PartnerOwner: GraphQLFieldConfig<void, ResolverContext> = {
  type: PartnerOwnerType,
  description: "An authenticated Partner meant to be consumed by the owner",
  args: {
    id: {
      type: new GraphQLNonNull(GraphQLString),
  ...
  resolve: (_root, { id }, { partnerOwnerLoader }, info) => {
    if (!partnerOwnerLoader) {
      throw new Error(
    ...
    if (
      ...
      return partnerOwnerLoader(id)
    }
    return { id, _id: id }
  },
}

export default PartnerOwner
