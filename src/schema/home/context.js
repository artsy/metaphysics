...
export const moduleContext = {
  popular_artists: ({ rootValue: { deltaLoader } }) => popularArtists(deltaLoader).then(trending => assign({}, trending, { context_type: "Trending" })),
  active_bids: () => null,
  followed_artists: ({ rootValue: { followedArtistsLoader } }) => followedArtistsLoader({ size: 9, page: 1 }).then(({ body }) => assign({}, body, { context_type: "FollowArtists" })),
  ...
  live_auctions: ({ rootValue: { salesLoader } }) => featuredAuction(salesLoader).then(sale => assign({}, sale, { context_type: "Sale" })),
  current_fairs: ({ rootValue: { fairsLoader } }) => featuredFair(fairsLoader).then(fair => assign({}, fair, { context_type: "Fair" })),
  followed_artist: ({ rootValue: { artistLoader }, params }) => artistLoader(params.followed_artist_id).then(artist => assign(
        {},
        {
  ...
  related_artists: ({ rootValue: { artistLoader }, params }) => Promise.all([
      artistLoader(params.related_artist_id),
      artistLoader(params.followed_artist_id),
    ]).then(([related_artist, follow_artist]) => assign(
        {},
        {
  ...
  generic_gene: ({ rootValue: { geneLoader }, params: { gene_id } }) => geneLoader(gene_id).then(gene => assign({}, gene, { context_type: "Gene" })),
}
...
export default {
  ...
  resolve: ({ key, display, params }, options, request, { rootValue }) => moduleContext[key]({ rootValue, params: params || {} }),
}
