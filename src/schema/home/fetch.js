...
export const featuredFair = fairsLoader => fairsLoader({
  size: 5,
  active: true,
...
export const activeSaleArtworks = lotStandingLoader => lotStandingLoader({
  live: true,
})
  .then(results => results.map(result => result.sale_artwork))
  .then(sale_artworks => map(sale_artworks, "artwork"))

export const featuredAuction = salesLoader => salesLoader({
  live: true,
  size: 1,
  sort: "timely_at,name",
}).then((sales) => {
...
export const featuredGene = followedGenesLoader => followedGenes(followedGenesLoader, 1).then((follows) => {
  if (follows.length) {
...
export const geneArtworks = (filterArtworksLoader, id, size) => filterArtworksLoader({
  gene_id: id,
  for_sale: true,
  size: 60,
}).then(({ hits }) => slice(shuffle(hits), 0, size))

export const relatedArtists = suggestedSimilarArtistsLoader => suggestedSimilarArtistsLoader({
...
export const popularArtists = deltaLoader => deltaLoader({
  method: "fetch",
  n: 9,
