...
export const gravityStitchingEnvironment = (
  ...
  const { ENABLE_UNSTITCHING_GRAVITY_USER_ADDRESS } = config

  // Define the user address extensions conditionally
  const userAddressExtensions = ENABLE_UNSTITCHING_GRAVITY_USER_ADDRESS
    ? null
    : gql`
        extend type Me {
          addressConnection(
  ...
  return {
    ...
    resolvers: ENABLE_UNSTITCHING_GRAVITY_USER_ADDRESS
      ? {}
      : {
          Me: {
            addressConnection: {
