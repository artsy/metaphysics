...
export function enhance(xs = [], source = {}) {
  return xs.map(x => {return assign({}, source, x)})
}
...
export const join = (by, xs) => {return compact(xs).join(by)}
...
export const toKey = (path, options = {}) => {return `${path}?${toQueryString(options)}`}
export const exclude = (values, property) => {return xs =>
  {return reject(xs, x => {return includes(values, x[property])})}}
export const stripTags = str => {
  if (!str) return ""
  return String(str).replace(/<\/?[^>]+>/g, "")
}
export const markdownToText = str => {return stripTags(formatMarkdownValue(str, "html"))}
