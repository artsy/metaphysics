...
export const total = (path, accessToken, options = {}) => {
  ...
  const parsedKey = toKey(
    urlObject.pathname,
    assign({}, existingOptions, options, {
      size: 0,
      total_count: 1,
    })
  )

  return gravity(parsedKey, accessToken).then(({ headers }) => ({
    body: {
...
const load = (path, options = {}) => {
  ...
  return totalLoader.load(key).then(response => response.total)
}
